(ns saic-networks.main
  "Kotodama WASM entrypoint for the Saic_networks clean-room actor (L4) — Clojure port.
  ...
  facts (`saic_networks.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "saic_networks")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "saicnetw_cas"
    :fields [:number :jurisdiction :status :filedAt]   :required [:number :jurisdiction]
    :coerce {}                            :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "saicnetw_par"
    :fields [:caseId :role :name]                      :required [:role :name]
    :coerce {}                            :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents" :id-prefix "saicnetw_doc"
    :fields [:caseId :type :contentRef]                :required [:type]
    :coerce {}                            :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents" :id-prefix "saicnetw_inc"
    :fields [:type :jurisdiction :occurredAt]         :required [:type :jurisdiction]
    :coerce {}                            :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "saicnetw_wat"
    :fields [:name :program :entries]                  :required [:name :program]
    :coerce {:entries :int}               :refs {}}
   {:entity "Sanction"  :plural "sanctions" :id-prefix "saicnetw_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"saic_networks.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "saic_networks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :saic_networks-compat/ready)
